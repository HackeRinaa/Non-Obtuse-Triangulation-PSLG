...
void parseInput(const nlohmann::json& input, std::vector<Point>& points, std::vector<std::pair<int, int>>& constraints) {
    ...
    // Check if 'additional_constraints' key exists and if it empty
    ...
    if (input.contains("additional_constraints")) {
        ...
        // Check if additional_constraints is empty
        if (!additional_constraints.empty()) {
            ...
            // Handle the case where additional_constraints is empty
            // For example, you might want to log or print a message
...
void insertSteinerPoint(CDT& cdt, CDT::Face_handle face) {
    ...
    if (obtuse_index != -1) {
        ...
        // If no obtuse angle is found, fall back to a different insertion strategy : circumcenter
